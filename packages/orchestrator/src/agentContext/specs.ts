...
export const ENDPOINT_SPECS: HttpEndpointSpec[] = [
  ...
  },
  {
    ...
    method: 'POST',
    path: '/api/workspaces',
    summary: 'Create a named persistent workspace: validates the repo, `git worktree add .modes-workspaces/<name> -b modes-ws/<name>`, records the current HEAD as baseRef, and blind-copies a root `.env` into the worktree (cp -n — contents never read). No name → the generator picks one (`brisk-otter`).',
    requiresToken: true,
    body: {
      repoPath: { type: 'string', required: true, description: 'absolute path to a git repo' },
      name: { type: 'string', description: '/^[a-z0-9][a-z0-9-]{0,39}$/ — becomes the worktree dir and branch suffix; generated when omitted' },
      cli: { type: 'string', description: "agent CLI for prompt runs (default 'kimi')" },
    },
    examples: ['curl -X POST -H "x-modes-token: $TOKEN" -H "content-type: application/json" -d \'{"repoPath":"/abs/repo","name":"billing-redesign"}\' http://127.0.0.1:4177/api/workspaces'],
    notes: [TOKEN_NOTE, '201 returns the workspace record. 400 when the path is not a git repo or the name is invalid; 409 when the name is taken.'],
  },
  {
    kind: 'http',
    method: 'GET',
    path: '/api/workspaces',
    summary: 'List all workspaces (runs summarized as a count).',
    requiresToken: true,
    examples: ['curl -H "x-modes-token: $TOKEN" http://127.0.0.1:4177/api/workspaces'],
    notes: [TOKEN_NOTE],
  },
  {
    kind: 'http',
    ...
    summary: 'Workspace detail: the full record incl. run history, plus `diff` — { committed: git diff baseRef...HEAD, uncommitted: worktree state incl. untracked files }.',
    requiresToken: true,
    params: { id: { type: 'string', required: true, description: 'workspace id' } },
    examples: ['curl -H "x-modes-token: $TOKEN" http://127.0.0.1:4177/api/workspaces/WS_ID'],
    notes: [TOKEN_NOTE],
  },
  {
    kind: 'http',
    method: 'POST',
    path: '/api/workspaces/:id/prompt',
    summary: 'Send a prompt to the workspace\'s agent: `cli -p <text>` with cwd = the workspace\'s worktree, resuming the stored kimi session (`-r <sessionId>`) when there is one. Output streams under a run-N label; on completion the parsed session id is stored back, flagged sessionRenewed when the CLI silently replaced a dead session.',
    requiresToken: true,
    params: { id: { type: 'string', required: true, description: 'workspace id' } },
    body: { text: { type: 'string', required: true, description: 'the prompt, non-empty' } },
    examples: ['curl -X POST -H "x-modes-token: $TOKEN" -H "content-type: application/json" -d \'{"text":"add a health endpoint"}\' http://127.0.0.1:4177/api/workspaces/WS_ID/prompt'],
    notes: [
      TOKEN_NOTE,
      '202 { id, lane, sessionId, resumed } on accept. HARD busy lock: 409 while a run is live — two concurrent resumes of one session corrupt the CLI\'s session record.',
      'Only kimi resumes; other CLIs always spawn a fresh session.',
    ],
  },
  {
    kind: 'http',
    ...
    summary: 'SSE stream of workspace state, same shape as the task events route: current state immediately, an event per change, `event: lane_output` batches for live run output. Closes when the workspace settles back to idle.',
    requiresToken: true,
    params: {
      id: { type: 'string', required: true, description: 'workspace id' },
      token: { type: 'string', description: 'query-param alternative to the header — EventSource cannot set headers' },
    },
    examples: ['curl -N "http://127.0.0.1:4177/api/workspaces/WS_ID/events?token=$TOKEN"'],
    notes: [TOKEN_NOTE, 'Reconnecting readers catch up on run output via GET /api/workspaces/:id/runs/:lane/output?offset= instead of replaying SSE.'],
  },
  {
    kind: 'http',
    method: 'GET',
    path: '/api/workspaces/:id/runs/:lane/output',
    summary: 'Catch-up read of one run\'s raw stream file from a byte offset; same contract as the task lane output route.',
    requiresToken: true,
    params: {
      id: { type: 'string', required: true, description: 'workspace id' },
      lane: { type: 'string', required: true, description: 'run label: run-1, run-2…' },
      offset: { type: 'number', description: 'byte offset from a previous response (default 0); an offset past EOF restarts from 0' },
    },
    examples: ['curl -H "x-modes-token: $TOKEN" "http://127.0.0.1:4177/api/workspaces/WS_ID/runs/run-1/output?offset=0"'],
    notes: [TOKEN_NOTE, '404 when the console runs without a lane stream hub.'],
  },
  {
    kind: 'http',
    method: 'DELETE',
    path: '/api/workspaces/:id',
    summary: 'Tear a workspace down: `git worktree remove --force` the worktree, `git branch -D` its branch, then unregister the record. A half-failed cleanup keeps the record and reports the failures so the delete can be retried.',
    requiresToken: true,
    params: { id: { type: 'string', required: true, description: 'workspace id' } },
    examples: ['curl -X DELETE -H "x-modes-token: $TOKEN" http://127.0.0.1:4177/api/workspaces/WS_ID'],
    notes: [TOKEN_NOTE, '409 while a run is live.'],
  },
];
